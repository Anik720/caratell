...
import { setRedirect } from 'app/store/appstore/authSlice';
...
class FuseAuthorization extends Component {
  ...
  redirectRoute() {
    const { location, userRole, navigate, dispatch } = this.props;
    const { pathname } = location;
    ...
    if (!userRole || userRole.length === 0) {
      // dispatch(setRedirect({ redirectFlag: true, redirectTo: "/login" }));
      // navigate({
      //   pathname: '/login',
      // });
      ...
      // });
      // dispatch(setRedirect({ redirectFlag: true, redirectTo: loginRedirectUrl }));
...
function mapStateToProps({ appstore }) {
  return {
    userRole: appstore.auth.appUser.roles,
  };
}
